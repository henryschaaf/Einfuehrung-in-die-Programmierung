...
int sum_of_doubly_divisibles(int n, int l, int m)
{
    ...
    while (counter < n)
    {
        if (is_multiple_of(i, m) == 1 && is_multiple_of(i, l) == 1)
        {
            sum += i;
            counter++;
        }
...
{
    int max_x = canvas_width(c) - 1;
    int max_y = canvas_height(c) - 1;

    c = canvas_set_black(c,max_x , 0);
    c = canvas_set_black(c, max_x, max_y);
    c = canvas_set_black(c, 0, max_y);
...
{
    for (int i = 1; i <= canvas_height(c); i++)
    {
        for (int j = 0; j <= i - 1 && j < canvas_width(c); j++)
        {
            c = canvas_set_black(c, j, canvas_height(c) - i);
        }
    }
...
Canvas draw_horizontal_line(Canvas c, int x, int y, int length)
{
    for (int i = 0; i < length; i++)
    {
        if (i + x < canvas_width(c) && 0 <= i + x)
            c = canvas_set_black(c, i + x, y);
    }
    return c;
}
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height)
{
    ...
    {
        if (0 <= y - i && y - i < canvas_height(c))
        {
            c = draw_horizontal_line(c, x, y - i, width);
        }
    }
