...
bool uint8_array_equality(uint8_t* expected, uint8_t* got, size_t array_length){
    for(int i = 0; i < array_length; i++){
        if(expected[i] != got[i]){
            return false;
        }
...
void assert_uint8_array(Tester *t, uint8_t* expected, uint8_t* got, size_t array_length, const char *context){
    if (!uint8_array_equality(expected, got, array_length)) {
        const char* s_expected = format_uint8_array(expected, array_length);
        const char* s_got = format_uint8_array(got, array_length);
        handle_assertion_failure(t, s_expected, s_got, context);
        free((char*) s_expected);
        free((char*) s_got);
    }
